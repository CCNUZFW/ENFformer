...
%首先根据fram_num_or_len,与 sig_len_max 计算出fram_len帧长 fram_num帧数
if fram_num_or_len=='fram_len'
    fram_len = fram_parameter;%给的帧长
    fram_num = ceil(sig_len_max/fram_len);%计算帧数
elseif fram_num_or_len=='fram_num'
    fram_num = fram_parameter;%给的帧数
    fram_len = ceil(sig_len_max/fram_num);%计算帧长
end

len_sig = length(signal);%信号长度
% 已经获得了 fram_len 与fram_num  
% 获取未经过pad 的矩阵

% 先计算出每帧共多少个点， 用floor 多余的点每帧给一个 
% 每帧多少个样点
per_fram_sample_num = floor(len_sig/fram_num);%每帧多少个点
%前面remain_sample_num个frame比后面的fram_num-remain_sample_num帧要多一个点
remain_sample_num = len_sig-per_fram_sample_num*fram_num;%信号多出来多少点
...
% 当==0时 每帧刚好perfram个点，获取矩阵shape=(per_fram_sample_num,fram_num)
    ...
    no_pad_matrix = [];
    for i=1:fram_num%遍历帧数进行分帧并进行存储
...
% 需要进行拼接 ，先获取前remain_sample_num 帧 shape=(remain_sample_num，fram_num)
    signal_need_1 = signal(1:(per_fram_sample_num+1)*remain_sample_num)'; %转置，获取信号部分数据
    no_pad_matrix_1=[];
    for i=1:remain_sample_num%将获取的数据分帧得到remain_sample_num 帧，此时帧长为per_fram_sample_num+1
        row_sample = signal_need_1((i-1)*(per_fram_sample_num+1)+1:i*(per_fram_sample_num+1));
        no_pad_matrix_1=[no_pad_matrix_1;row_sample];
    end
    
    signal_need_2 = signal((per_fram_sample_num+1)*remain_sample_num+1:len_sig)';%获取信号剩余数据点
    no_pad_matrix_2=[];
    for i=1:(fram_num-remain_sample_num)%进行分帧，此时帧长为per_fram_sample_num
